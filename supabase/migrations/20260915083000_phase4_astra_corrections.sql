-- Phase 4 Astra design-review corrections.
-- Corrects derived analytics semantics without mutating historical MealEntry snapshots.
-- Keeps SECURITY INVOKER and existing RLS ownership boundaries.
...
begin
  ...
  aggregated as (
    select
      ...
      coalesce(sum(en.amount) filter (
        where en.amount is not null
          and en.item_type in ('ingredient', 'product', 'estimated_dish')
  ...
  select
    ...
    (
      a.record_complete
      and a.entry_count > 0
      and a.missing_entry_count = 0
    ) as eligible_for_reference,
    case
      when a.entry_count = 0 then 'not_applicable'
...
comment on function public.get_nutrition_daily_summary(date, date)
  is 'Phase 4 owner-scoped derived nutrition summary. Empty-entry days are not comparison-eligible; Batch contribution is left source-unclassified; missing nutrient values are never converted into complete zero totals.';
