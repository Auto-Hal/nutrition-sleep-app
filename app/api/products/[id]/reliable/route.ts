...
const patchNutrientSchema = z.object({
  code: z.string().refine(
    (value) => NUTRIENT_DEFINITIONS.some((definition) => definition.code === value),
    "unknown nutrient",
  ),
...
const schema = operationSchema.and(z.object({
  ...
  nutrients: z.array(patchNutrientSchema).max(NUTRIENT_DEFINITIONS.length),
  replace_all_nutrients: z.boolean(),
  confirm_verified_overwrite: z.boolean(),
})).superRefine((value, context) => {
  ...
  if (value.replace_all_nutrients) {
    for (const nutrient of value.nutrients) {
      if (
        ...
        context.addIssue({ code: "custom", message: "full replacement requires complete nutrient tuples" });
        break;
      }
    }
  }
...
export async function PATCH(request: Request, context: Context) {
  ...
  const result = await createUserClient(session.accessToken).rpc("update_product_item_reliable_v2", {
