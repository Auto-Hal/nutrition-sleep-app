...
const schema = z.object({
  ...
  expected_revision: z.number().int().positive(), batch_id: z.string().uuid(),
...
export async function PATCH(request: Request, context: Context) {
  ...
  if (!z.string().uuid().safeParse(id).success || !parsed.success || parsed.data.batch_id !== id) {
    return NextResponse.json({ error: "入力内容を確認してください。", error_code: "invalid_request" }, { status: 400 });
  }
  const d = parsed.data;
  const result = await createUserClient(session.accessToken).rpc("update_batch_v2", {
    p_operation_id: d.operation_id, p_contract_version: d.contract_version, p_intent_created_at: d.intent_created_at,
    p_batch_id: id, p_expected_revision: d.expected_revision, p_name: d.name, p_dish_name: d.dish_name,
    p_servings: d.servings, p_serving_unit: d.serving_unit, p_components: d.components,
  });
