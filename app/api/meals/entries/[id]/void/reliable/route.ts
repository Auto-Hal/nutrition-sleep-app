...
const schema = z.object({
  ...
  expected_meal_revision: z.number().int().positive(),
});
...
export async function POST(request: Request, context: Context) {
  ...
  const d = parsed.data;
  const result = await createUserClient(session.accessToken).rpc("void_meal_entry_v2", {
    p_operation_id: d.operation_id, p_contract_version: d.contract_version, p_intent_created_at: d.intent_created_at,
    p_entry_id: id, p_expected_meal_revision: d.expected_meal_revision,
  });
  if (result.error) return reliableMutationError(result.error, "食事記録の取消を同期できませんでした。");
